...
module.exports = function(connect) {
	...
	if(this.config.use === "v4") {
	    ...
	    this.server.bind(this.config.multicastPort, function() {
		return null;
	    }.bind(this));
	...
	this.log.info("Got new session:", messageObject);
	...
	this.hostDb.remove({ address: hostInfo.address },function(err, docs) {});
	return null;
    };

    ReplicatedStore.prototype.destroySession = function(messageObject, hostInfo) {

	var message = this.serializer.serialize(messageObject);
	if(!message) {
	    return null;
	}
	...
	if(! options.multicast) {
	    this.log.debug("Setting default:", defaultMulticast);
